...
import { detectTemplate, applyTemplate } from './figma-templates.js';
...
export function collectIllustrationNodeIds(node, result = []) {
  if (!node || node.visible === false || node.type === 'TEXT') return result;

  const b = node.absoluteBoundingBox;
  const largeEnough = !b || (b.width >= 16 && b.height >= 16);

  // 노드 이름에 'icon' 또는 'img' 포함 → 내부 구조 무관하게 단일 이미지로 export
  const nameLower = (node.name || '').toLowerCase();
  if ((nameLower.includes('icon') || nameLower.includes('img')) && largeEnough) {
    result.push(node.id);
    return result;
  }
  ...
  for (const c of (node.children || [])) {
    collectIllustrationNodeIds(c, result);
  }
  return result;
}
...
export async function figmaAccurateMarkup(figmaUrl, markupType = 'html') {
  ...
  let allCdnUrls = null; // IMAGE fill 발견 시 지연 조회
  ...
  for (const nid of nodeIds.slice(0, 3)) {
    ...
    if (imageRefs.size > 0) {
      if (allCdnUrls === null) {
        try { allCdnUrls = await fetchImageFillUrls(fileKey); } catch { allCdnUrls = {}; }
      }
    ...
    const templateKey = markupType !== 'react' ? detectTemplate(nodeData) : null;
    const result = templateKey
      ? applyTemplate(templateKey, nodeData, imageNodeMap, imageFillUrls)
      : convertFigmaNode(nodeData, markupType, imageFillUrls, new Set(), imageNodeMap);
