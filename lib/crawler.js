...
export async function crawl(url, selector = '') {
  ...
  try {
    ...
    try {
      ...
      // resp.text()는 fetch의 기본 utf-8 가정만 쓰고, Content-Type 헤더에 charset이 없거나
      // 실제로 euc-kr인 사이트에서 학교명·메뉴명이 깨진다(Playwright 실패 시에만 타는
      // 폴백이라도 인코딩 처리는 다른 크롤링 경로와 동일하게 맞춰야 한다).
      ...
      if (charset === 'utf-8') {
        ...
        if (replacementCount >= 3 && replacementCount / lenient.length > 0.01) charset = 'euc-kr';
      }
