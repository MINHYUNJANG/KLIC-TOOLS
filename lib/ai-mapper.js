    ...
    for (const msgs of [truncateForCerebras(messages), truncateForCerebras(truncateMessages(messages))]) {
      try {
        ...
        if (!resp.ok) {
          ...
          if ([400, 404, 413, 429].includes(resp.status)) break; // 404: 모델 미존재, 400: 잘못된 요청 → 다음 모델로
          throw lastError;
        }
...
function directMarkupHtml(html) {
  ...
  if (minLvl >= 3) {
    ...
    const isPpOrVpD = /개인정보\s*(?:처리|보호)\s*방침|영상\s*정보\s*처리\s*방침/.test($('body').text());
  ...
  // 개인정보·영상정보처리방침 타이틀 + 제N조 패턴 p 태그 승격 (테이블 경로도 동일 처리)
  $('p').each((_, el) => {
    const $el = $(el);
    const text = $el.text().trim();
    if (/개인정보\s*(?:처리|보호)\s*방침/.test(text) && text.length < 100 && /방침\s*$/.test(text)) {
