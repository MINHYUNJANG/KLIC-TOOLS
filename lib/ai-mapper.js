...
2. h4(tit-st contents) 타이틀 바로 아래의 내용은 별도 래퍼 div로 감싸지 말고 그대로 출력
...
function postProcessMarkup(html) {
  ...
  // 5) 조항(제N조) p → h4.tit-st contents (최상위 + h4.contents 본문 내 단독 strong p도 처리)
  $('p').each((_, p) => {
    const $p = $(p);
    const $ch = $p.children();
    const text = $p.text().trim();
    const isClause = /^제\s*\d+\s*조/.test(text) && text.length < 80;
    const isOnlyStrong = $p.prevAll('h1,h2,h3,h4,h5,h6').first().is('h4.contents') &&
  ...
  // 안전장치: AI가 지시를 무시하고 div.indent로 감쌌더라도 항상 벗겨서 출력에 남기지 않는다.
  $('.indent').each((_, div) => {
    const $div = $(div);
...
function postProcessFooterMarkup(html) {
  ...
  $('p').each((_, el) => {
    ...
    const text = $el.text().trim();
    if (!NUMBERED_SEC_RE.test(text) || text.length > 150 || /다\.$/.test(text)) return;
    if ($el.parents('ul,ol,li,table,.box-st').length) return;
    if ($el.prevAll('h1,h2,h3,h4,h5,h6').first().is('h4.contents')) return;
  ...
  // 9.5) h4.tit-st.contents 바로 다음, 리스트/표 바로 앞에 나오는 단독 안내
