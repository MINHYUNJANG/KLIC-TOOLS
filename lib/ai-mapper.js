...
export async function footerAutoMarkup(crawledData) {
  ...
  // CP 타이틀 폴백: 첫 번째 h3.tit-st.section에 "저작권" 관련 텍스트가 없으면 자동 삽입
  if (isCP) {
    const $cp = cheerio.load(ppResult);
    const firstH3 = $cp('h3.tit-st.section').first();
    const hasCPTitle = firstH3.length && /저작권/.test(firstH3.text());
    if (!hasCPTitle) {
      // 기존 출력에서 "저작권"+"보호" 텍스트 요소 승격 시도 (h2→h3.section 변환 미처리 폴백)
      const existing = $cp('p, h2, h3, h4').filter((_, el) => {
        const t = $cp(el).text().trim();
        return /저작권/.test(t) && /보호/.test(t) && t.length < 150;
      }).first();
      if (existing.length) {
        existing.replaceWith(`<h3 class="tit-st section">${existing.html()}</h3>`);
      } else {
        let schoolName = crawledData.school_name || '';
        if (!schoolName) {
          const bodyText = $cp('body').text();
          const m = bodyText.match(/([가-힣]{2,15}(?:초등학교|중학교|고등학교|특수학교|대학교|학교))/);
          schoolName = m ? m[1] : '';
        }
        const titleText = schoolName ? `${schoolName} 저작권 보호 지침` : '저작권 보호 지침';
