    ...
    for (const msgs of [truncateForCerebras(messages), truncateForCerebras(truncateMessages(messages))]) {
      try {
        ...
        if (!resp.ok) {
          ...
          if ([429, 413, 400].includes(resp.status)) break; // 400도 다음 모델로
          throw lastError;
        }
...
function directMarkupHtml(html) {
  ...
  if (minLvl >= 3) {
    // PP 콘텐츠에서 번호형 h3이 최상위면 정규화 스킵
    // → h3은 h4.tit-st.contents로 매핑되어야 하며, 타이틀은 footerAutoMarkup 폴백이 보완
    const NUMBERED_RE_D = /^\d+\.\s+[가-힣A-Za-z]/;
    const isPpD = /개인정보\s*(?:처리|보호)\s*방침/.test($('body').text());
    const numberedH3D = $('h3').toArray().some(el => NUMBERED_RE_D.test($(el).text().trim()));
    if (!(isPpD && minLvl === 3 && numberedH3D)) {
  ...
  // 개인정보처리방침 타이틀 + 제N조 패턴 p 태그 승격 (테이블 경로도 동일 처리)
  $('p').each((_, el) => {
    const $el = $(el);
    const text = $el.text().trim();
    if (/개인정보\s*(?:처리|보호)\s*방침/.test(text) && text.length < 100 && /방침\s*$/.test(text)) {
...
function preprocessFooterHtml(html) {
  ...
  const minLvl = [1,2,3,4,5,6].find(l => $(`h${l}`).length > 0) ?? 0;
  if (minLvl >= 3) {
    const isPpCtx = PRIVACY_RE.test($('body').text());
    const numberedH3Exists = $('h3').toArray().some(el => NUMBERED_SEC_RE.test($(el).text().trim()));
    if (!(isPpCtx && minLvl === 3 && numberedH3Exists)) {
...
function postProcessFooterMarkup(html) {
  ...
  // 9) h3.section 바로 아래 연속 p → div.box-st emp로 감싸기
  //    최상위 타이틀(개인정보처리방침·일반 모두) 직후 안내 문단 처리
  //    "다." 문장 경계에 <br> 삽입 (조건 없이 p 전체 수집, 비p 요소에서 중단)
  $('h3.section').each((_, h3) => {
    const $h3 = $(h3);
    let cursor = $h3.next();
    // box-st emp가 바로 다음이면 이미 처리됨
    if (cursor.is('.box-st.emp')) return;
    // p가 아닌 비헤딩 요소(라벨링 박스, ul 등) 건너뛰기
    while (cursor.length && !cursor.is('p') && !cursor.is('h3,h4,h5,h6,.box-st.emp,.indent')) {
      cursor = cursor.next();
    }
    if (!cursor.length || !cursor.is('p')) return;
    const group = [];
...
export async function footerAutoMarkup(crawledData) {
  ...
  if (isPP) {
    ...
    // h3.section 직후 p → box-st emp (directMarkupHtml 직후 즉시 처리, postProcess 간섭 전)
    const $dm = cheerio.load(result);
    $dm('h3.section').each((_, h3) => {
      const $h3 = $dm(h3);
      let cursor = $h3.next();
      // box-st emp가 바로 다음이면 이미 처리됨
      if (cursor.is('.box-st.emp')) return;
