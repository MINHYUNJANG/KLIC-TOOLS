...
9. 개인정보처리방침 최상위 타이틀(h3.tit-st.section) 바로 아래 안내 문구:
   - "OO학교는 ~" 또는 학교명으로 시작하는 안내 문장:
    ...
    while (i < pArr.length) {
      ...
      while (j < pArr.length) {
        ...
        if (PAREN_NUM_RE.test(currTxt) && currTxt.length <= 80) {
          ...
          const $next = $(pArr[j + 1]);
          if ($next.length && !PAREN_NUM_RE.test($next.text().trim())) {
...
function postProcessFooterMarkup(html) {
  ...
  // 9) h3.tit-st.section 바로 아래 연속 p → div.box-st emp로 감싸기
  //    "다." 문장 경계에 <br> 삽입 (조건 없이 p 전체 수집, 비p 요소에서 중단)
  $('h3.tit-st.section').each((_, h3) => {
    const $h3 = $(h3);
    if ($h3.next().is('[class*="box-st"]')) return;
    const group = [];
    let cursor = $h3.next();
