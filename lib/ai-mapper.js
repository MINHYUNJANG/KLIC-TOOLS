...
function buildHtmlForAI(html) {
  ...
  // 3.6. dl: 번호형 dt + 대응 dd가 2개 이상 → ul.bu-st1.list (AI가 h4로 변환하지 않도록 선처리)
  $('dl').each((_, dl) => {
    const $dl = $(dl);
    const dtEls = $dl.children('dt').toArray();
    const ddEls = $dl.children('dd').toArray();
    const numberedDts = dtEls.filter(dt => /^\d+\.\s+[가-힣A-Za-z]/.test($(dt).text().trim()));
    if (numberedDts.length >= 2 && (ddEls.length === 0 || ddEls.length >= numberedDts.length * 0.5)) {
      ...
      $dl.children().toArray().forEach(child => {
        ...
        const $child = $(child);
        if (childTag === 'dt') {
          if (pendingDt) items.push(`<li>${pendingDt}</li>`);
          pendingDt = ($child.html() || '').trim();
        } else if (childTag === 'dd') {
          const ddHtml = ($child.html() || '').trim();
          items.push(`<li>${pendingDt ? pendingDt + (ddHtml ? '<br>' + ddHtml : '') : ddHtml}</li>`);
          pendingDt = '';
        }
      });
      if (pendingDt) items.push(`<li>${pendingDt}</li>`);
...
function directMarkupHtml(html) {
  ...
  function processEl(el) {
    ...
    if (tag === 'dl') {
      // 번호형(제N조 제외) dt + 대응 dd가 2개 이상이면 ul.bu-st1.list로 변환
      ...
      const numberedDts = dtEls.filter(dt => {
        ...
        return /^\d+\.\s+[가-힣A-Za-z]/.test(t) && t.length < 80;
      });
      if (numberedDts.length >= 2 && (ddEls.length === 0 || ddEls.length >= numberedDts.length * 0.5)) {
        const items = [];
        let pendingDt = '';
        ...
        if (pendingDt) items.push(`<li>${pendingDt}</li>`);
        return items.length ? `<ul class="bu-st1 list">${items.join('')}</ul>` : '';
      }
...
function preprocessFooterHtml(html) {
  ...
  if (PRIVACY_RE.test($('body').text()) || CP_RE_PRE.test($('body').text()) || VP_RE_PRE.test($('body').text())) {
    $('dl').each((_, dl) => {
      ...
      // 번호형(제N조 제외) dt + 대응 dd가 2개 이상 → ul.bu-st1.list로 직접 변환 (h3 승격 방지)
      const ddEls = $dl.children('dd').toArray();
      ...
      if (numberedDts.length >= 2 && (ddEls.length === 0 || ddEls.length >= numberedDts.length * 0.5)) {
        ...
        if (pendingDt) items.push(`<li>${pendingDt}</li>`);
        if (items.length) {
          $dl.replaceWith(`<ul class="bu-st1 list">${items.join('')}</ul>`);
          return;
        }
      }
      ...
      $dl.children().toArray().forEach(child => {
        ...
        if (childTag === 'dt') {
          ...
          if (isClause || isNumbered) {
            ...
            if (ddHtml) replacements += /<(?:table|div|ul|ol|h[1-6]|p)\b/i.test(ddHtml) ? ddHtml : `<p>${ddHtml}</p>`;
          } else {
