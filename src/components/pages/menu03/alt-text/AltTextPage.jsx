...
import InspectionHelpModal from '../components/InspectionHelpModal';
    ...
    <div className="crawl-page">
      <div className="crawl-page-inner">
        <div className="crawl-title-row">
          ...
          <button type="button" className="crawl-help-btn" onClick={() => setShowHelp(true)} aria-label="대체텍스트 생성 사용방법 열기">?</button>
        </div>
        ...
        {showHelp && (
          <InspectionHelpModal
            title="대체텍스트 생성 사용 순서"
            onClose={() => setShowHelp(false)}
            steps={[
              { title: '입력 방식 선택', description: '페이지 URL 입력 또는 이미지 직접 업로드 방식을 선택합니다.' },
              { title: '이미지 분석', description: 'URL은 이미지 최대 10개, 업로드는 파일 최대 5개까지 AI로 분석합니다.' },
              { title: '대체텍스트 검토', description: '생성된 대체텍스트를 확인하고 필요한 문구를 직접 수정합니다.' },
              { title: '마크업 복사', description: '일반 이미지는 alt, 복잡한 이미지는 aria-describedby와 sr-only 권장 마크업을 복사합니다.' },
            ]}
          />
        )}
