    ...
    <main className="audit-page">
      <section className="audit-page-header">
        <div className="crawl-title-row">
          ...
          <button type="button" className="crawl-help-btn" onClick={() => setShowHelp(true)} aria-label="웹표준검사 사용방법 열기">?</button>
        </div>
      ...
      {showHelp && (
        <InspectionHelpModal
          ...
          steps={[
            { title: '브라우저 선택', description: '증적을 생성할 브라우저를 1개 이상, 최대 5개까지 선택합니다.' },
            { title: '검사 URL 입력', description: '검사할 페이지 URL을 입력하고 검사를 시작합니다.' },
            { title: '오류와 조치방법 확인', description: 'W3C HTML·CSS 오류 및 경고와 항목별 조치방법을 확인합니다.' },
            { title: '증적보고서 저장', description: '브라우저별 캡처와 검사 결과가 포함된 HWPX 보고서를 다운로드합니다.' },
          ]}
        />
      )}
