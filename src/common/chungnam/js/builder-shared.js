/**
 * builder-shared.js ??KlicTools 鍮뚮뜑 怨듯넻 ?좏떥由ы떚
 * 紐⑤뱺 鍮뚮뜑(肄섑뀗痢?議곗쭅???ㅼ떆?붽만 ???먯꽌 怨듭쑀 ?ъ슜
 * window.KlicBuilderShared ?쇰줈 ?몄텧
 */
(function () {
	const MARKUP_EMPTY_CSS = '/* ?곸슜???ㅽ????놁쓬 */';
	...
	function initBuilderHeader() {
		...
		if (expandBtn) {
			setExpandButtonState(expandBtn, window.parent !== window);
		}
		if (window.parent !== window) {
			window.parent.postMessage({ type: 'builderExpandToggle', expanded: true }, '*');
		}
		...
		const showStep = index => {
			...
			callout.innerHTML = `
				...
				<div class="tour-callout-actions">
					<button type="button" class="tour-skip">嫄대꼫?곌린</button>
					<div class="tour-nav">
						${!isFirst ? '<button type="button" class="tour-prev">?댁쟾</button>' : ''}
						<button type="button" class="tour-next${isLast ? ' is-last' : ''}">${isLast ? '?꾨즺' : '?ㅼ쓬'}</button>
					</div>
				</div>`;
