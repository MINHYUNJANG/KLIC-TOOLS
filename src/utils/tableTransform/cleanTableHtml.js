...
import { processTextContentNormal, processTextContentColor } from './textProcessor';
import { processTableOnlyNormal, processTableOnlyColor } from './tableProcessor';
import { convertCircleToArabic, MARKER_TYPES, EXCLUDE_MARKER_REGEXES, RE_WHITESPACE } from './constants';

export { updateStylesOnly } from './styleUpdater';
        ...
        while (changed) {
            ...
            for (let i = 0; i < children.length; i++) {
                ...
                if (afterEl && (afterEl.tagName === 'OL' || afterEl.tagName === 'UL')) {
                    ...
                    let target = null;
                    if (bMarker && bMarker === deepLastMarker) target = deepestList;
                    else if (bMarker) target = _findAncestorListByMarker(deepestList, listA, bMarker);
