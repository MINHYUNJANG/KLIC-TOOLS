...
function popFullmenu() {	
    ...
    if ($('.popFullmenu').length > 0) {
        // 제목("전체메뉴")은 별도 엘리먼트 대신 depth01 > ul::before로 처리 (CSS 참고)
		...
		// depth02를 li 밖으로 꺼내 별도 컨테이너(depth02Wrap)에 모음
		// (li 안에 중첩된 채로 두면, 좌측 사이드바(ul)에 sticky/fixed 등을 걸 때
		//  depth02의 위치기준(containing block)이 ul로 잘못 잡혀버리는 문제가 생김)
    ...
    // depth02가 li 밖으로 분리되어 있으므로, li의 active 상태가 바뀔 때마다
    // data-menu-idx가 일치하는 depth02(depth02Wrap 안)도 같이 active 처리
    ...
    // 접근성 : 키보드 포커스 흐름
    // depth01 링크에서 Enter → 새로 활성화된 depth02의 첫 항목으로 포커스 이동
    // depth02 마지막 항목에서 Tab → 다음 depth01 링크로 (없으면 닫기 버튼)
    // depth02 첫 항목에서 Shift+Tab → 자기 depth01 링크로
    ...
    $(document).on('keydown', '.popFullmenu .depth01 > ul > li > a', function(e) {
        ...
        // click 핸들러(active 전환/depth02 동기화)가 먼저 끝나도록 다음 tick에서 포커스 이동
        setTimeout(function() {
