...
/* -------------------------------------------------------------
   04_상징 공통 악보 다운로드 버튼 이벤트
   ------------------------------------------------------------- */
$(document).on('click', '#songDwnBtn', function () {
	const $img = $(this).closest('.box').find('.songWrite img');
...
/* -------------------------------------------------------------
   04_상징A_Slide | 좌측 네비게이션 고정 + 콘텐츠 영역 자체 스크롤
   (.list-wrap 내부 스크롤에만 반응 — 그 밖(페이지)에서는 일반 스크롤)
   ------------------------------------------------------------- */
$(function () {
	...
	let $nav = $();
	let $navBtns = $();
	if (!$wrap.parent().hasClass("symbol-nav-wrap")) {
	...
	/* 박스가 1개뿐이면 내부 스크롤 없이 그대로 보여준다 (CSS .dis-scroll 참고). 2개 이상이면 항상 내부 스크롤 박스를 유지한다. */
	$wrap.toggleClass("dis-scroll", $boxes.length <= 1);

	if (!$navBtns.length) return;

	/* root를 .list-wrap으로 지정 → 그 영역 내부 스크롤에만 반응하고,
	   페이지(바깥) 스크롤에는 반응하지 않는다. */
